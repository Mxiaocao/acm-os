    ...
    <main className="review-shell" ref={mainRef} tabIndex={-1}>
      <header className="review-header">
        <div>
          ...
          <h1>{focus ? displayProblemTitle("", focus.title) : t("review.focusWorkspace")}</h1>
        </div>
...
function RewardPage() {
  ...
  const [customRewards, setCustomRewards] = useState<CustomRewardDto[]>([]);
  const [customRewardsLoading, setCustomRewardsLoading] = useState(false);
  const [customRewardsError, setCustomRewardsError] = useState(false);
  ...
  const refreshRewardAccount = useCallback(async () => { await loadAccount(); }, [loadAccount]);

  const loadCustomRewards = useCallback(async () => {
    setCustomRewardsLoading(true);
    setCustomRewardsError(false);
    try {
      setCustomRewards(await listCustomRewards());
    } catch {
      setCustomRewardsError(true);
    } finally {
      setCustomRewardsLoading(false);
    }
  }, []);
  ...
  const loadActivation = useCallback(async () => {
    ...
    try {
      ...
      if (state.active) await Promise.all([loadAccount(), loadCustomRewards()]);
    } catch {
      setActivationView("error");
    }
  }, [loadAccount, loadCustomRewards]);
    ...
    {activationView === "active" ? (
      <>
        ...
        <RewardTransactions
          ...
          customRewardsError={customRewardsError}
          customRewardsLoading={customRewardsLoading}
          managementSlot={
            <CustomRewardManagement
              customRewards={customRewards}
              customRewardsError={customRewardsError}
              customRewardsLoading={customRewardsLoading}
              onCustomRewardsRefresh={loadCustomRewards}
            />
          }
          onCustomRewardsRefresh={loadCustomRewards}
          onTransactionResolved={refreshRewardAccount}
        />
      </>
    ) : null}
    {confirmationOpen ? (
...
function CustomRewardManagement({ customRewards, customRewardsLoading, customRewardsError, onCustomRewardsRefresh }: {
  customRewards: CustomRewardDto[];
  customRewardsLoading: boolean;
  customRewardsError: boolean;
  onCustomRewardsRefresh: () => Promise<void>;
}) {
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (mutationPendingRef.current) return; setMutationError(null); const input = validate(name, coinCost); if (!input) return; mutationPendingRef.current = true; setPending(true); try { await createCustomReward(input); setName(""); setCoinCost(""); setInvalidFields({ name: false, coinCost: false }); await onCustomRewardsRefresh(); } catch { setMutationError(t("reward.changeError")); } finally { mutationPendingRef.current = false; setPending(false); } };
  ...
  return <details aria-labelledby="custom-rewards-heading" className="content-panel reward-management">
    <summary className="reward-section-summary">
      <div className="reward-management__summary-main"><h2 id="custom-rewards-heading">{t("reward.customRewards")}</h2><span className="reward-count" aria-label={`${customRewards.filter((reward) => reward.status === "active").length} active rewards`}>{customRewards.filter((reward) => reward.status === "active").length}</span></div>
      <div className="reward-management__toolbar">
        <label className="reward-archive-toggle" onClick={(event) => event.stopPropagation()}><input checked={showArchived} onChange={(event) => setShowArchived(event.currentTarget.checked)} type="checkbox" /> <span>{t("reward.showArchived")}</span></label>
        {!createExpanded ? <button aria-controls="custom-reward-create-form" aria-expanded={false} className="secondary-action reward-create-toggle" onClick={openCreateForm} type="button">{t("reward.newReward")}</button> : null}
      </div>
    </summary>
    <div className="reward-management__body">
    {mutationError ? <p aria-live="assertive" className="error-message" id="custom-reward-error" role="alert">{mutationError}</p> : null}
    <form aria-label={t("reward.create")} className="reward-create-form" id="custom-reward-create-form" noValidate onSubmit={create} hidden={!createExpanded}>
      <div className="reward-form-fields">
        <label>{t("reward.name")}<input aria-describedby={invalidFields.name ? "custom-reward-error" : undefined} aria-invalid={invalidFields.name} aria-label={t("reward.customNameAria")} onInput={(event) => { setName(event.currentTarget.value); if (invalidFields.name) setInvalidFields((current) => ({ ...current, name: false })); }} value={name} /></label>
        <label>{t("reward.coinCost")}<input aria-describedby={invalidFields.coinCost ? "custom-reward-error" : undefined} aria-invalid={invalidFields.coinCost} aria-label={t("reward.customCoinCostAria")} inputMode="numeric" onInput={(event) => { setCoinCost(event.currentTarget.value); if (invalidFields.coinCost) setInvalidFields((current) => ({ ...current, coinCost: false })); }} value={coinCost} /></label>
      </div>
      <div className="action-row reward-form-actions">{createExpanded ? <button className="secondary-action" disabled={pending} onClick={() => { setCreateExpanded(false); setName(""); setCoinCost(""); setInvalidFields({ name: false, coinCost: false }); setMutationError(null); }} type="button">{t("common.cancel")}</button> : null}<button className="primary-action" disabled={pending} type="submit">{t("reward.create")}</button></div>
    </form>
    {customRewardsLoading ? <p aria-live="polite">{t("reward.loadingCustom")}</p> : null}
    {customRewardsError ? <div role="alert"><p>{t("reward.customLoadError")}</p><button className="secondary-action" onClick={() => void onCustomRewardsRefresh()} type="button">{t("common.retry")}</button></div> : null}
    {!customRewardsLoading && !customRewardsError && visible.length === 0 ? <p>{showArchived ? t("reward.noCustomAll") : t("reward.noCustomActive")}</p> : null}
    {!customRewardsLoading && !customRewardsError && visible.length > 0 ? <ul className="reward-list">{visible.map((reward) => <li className="reward-item" key={reward.customRewardId}><div className="reward-item__details"><strong>{reward.name}</strong><span>{t("reward.coinCostValue", { cost: reward.coinCost })}</span>{reward.status === "archived" ? <span className="reward-item__status">{t("reward.archived")}</span> : null}</div>{reward.status === "active" ? <div className="action-row reward-item__actions"><button className="secondary-action" onClick={() => edit(reward)} type="button">{t("reward.edit")}</button><button className="danger-action reward-archive-action" onClick={(event) => { archiveTriggerRef.current = event.currentTarget; setArchiveTarget(reward); }} type="button">{t("reward.archive")}</button></div> : null}{editingId === reward.customRewardId ? <form className="reward-edit-form" noValidate onSubmit={(event) => void update(event, reward)}><div className="reward-form-fields"><label>{t("reward.name")}<input aria-describedby={invalidFields.name ? "custom-reward-error" : undefined} aria-invalid={invalidFields.name} aria-label={t("reward.editNameAria")} onInput={(event) => { setEditName(event.currentTarget.value); if (invalidFields.name) setInvalidFields((current) => ({ ...current, name: false })); }} value={editName} /></label><label>{t("reward.coinCost")}<input aria-describedby={invalidFields.coinCost ? "custom-reward-error" : undefined} aria-invalid={invalidFields.coinCost} aria-label={t("reward.editCoinCostAria")} onInput={(event) => { setEditCoinCost(event.currentTarget.value); if (invalidFields.coinCost) setInvalidFields((current) => ({ ...current, coinCost: false })); }} value={editCoinCost} /></label></div><div className="action-row reward-form-actions"><button className="primary-action" disabled={pending} type="submit">{t("reward.saveChanges")}</button><button className="secondary-action" disabled={pending} onClick={() => { setEditingId(null); setInvalidFields({ name: false, coinCost: false }); setMutationError(null); }} type="button">{t("common.cancel")}</button></div></form> : null}</li>)}</ul> : null}
    </div>
...
function RewardTransactions({ account, customRewards, customRewardsLoading, customRewardsError, managementSlot, onCustomRewardsRefresh, onTransactionResolved }: {
  account: RewardAccountSummaryDto | null;
  customRewards: CustomRewardDto[];
  customRewardsLoading: boolean;
  customRewardsError: boolean;
  managementSlot: ReactNode;
  onCustomRewardsRefresh: () => Promise<void>;
  onTransactionResolved: () => void;
}) {
  ...
  const refresh = async () => { await Promise.all([onCustomRewardsRefresh(), loadHistory()]); await onTransactionResolved(); };
  const confirm = async () => {
  ...
  const activeRewards = customRewards.filter((reward) => reward.status === "active");
  const [rewardsExpanded, setRewardsExpanded] = useState(false);
  const visibleRewards = rewardsExpanded ? activeRewards : activeRewards.slice(0, 6);
  ...
  return <>
    <section aria-labelledby="reward-actions-heading" className="content-panel reward-redemption"><div className="statement-heading-row"><h2 id="reward-actions-heading">{t("reward.redeemRewards")}</h2>{account ? <span className="reward-balance">{t("reward.coin")}: {account.coin}</span> : null}</div>{customRewardsLoading ? <p aria-live="polite">{t("reward.loadingCustom")}</p> : null}{customRewardsError ? <div role="alert"><p>{t("reward.customLoadError")}</p><button className="secondary-action" onClick={() => void onCustomRewardsRefresh()} type="button">{t("common.retry")}</button></div> : null}{!customRewardsLoading && !customRewardsError && activeRewards.length === 0 ? <p>{t("reward.noCustomActive")}</p> : null}{!customRewardsLoading && !customRewardsError && activeRewards.length > 0 ? <><ul className="reward-grid">{visibleRewards.map((reward) => { const insufficient = account !== null && account.coin < reward.coinCost; const descriptionId = "reward-insufficient-" + reward.customRewardId; return <li className="reward-card" key={reward.customRewardId}><strong>{reward.name}</strong><span>{t("reward.coinCostValue", { cost: reward.coinCost })}</span><button aria-describedby={insufficient ? descriptionId : undefined} aria-label={`${t("reward.redeem")} ${reward.name}`} className="primary-action" disabled={insufficient || pending} onClick={(event) => beginRedeem(reward, event)} type="button">{t("reward.redeem")}</button>{insufficient ? <span className="sr-only" id={descriptionId}>{t("reward.notEnoughCoin")}</span> : null}</li>; })}</ul>{activeRewards.length > 6 ? <button className="secondary-action reward-expand" onClick={() => setRewardsExpanded((expanded) => !expanded)} type="button">{rewardsExpanded ? t("reward.collapse") : t("reward.viewAll", { count: activeRewards.length })}</button> : null}</> : null}</section>
    {managementSlot}
    <details className="content-panel reward-history"><summary className="reward-section-summary"><h2 id="redemption-history-heading">{t("reward.redemptionHistory")}</h2><span>{history.length}</span></summary><div className="reward-history__body">{historyLoading ? <p aria-live="polite">{t("reward.historyLoading")}</p> : null}{historyError ? <div role="alert"><p>{t("reward.historyError")}</p><button className="secondary-action" onClick={() => void loadHistory()} type="button">{t("reward.retryHistory")}</button></div> : null}{!historyLoading && !historyError && history.length === 0 ? <p>{t("reward.noRedemptions")}</p> : null}{!historyLoading && !historyError && history.length > 0 ? <ul className="detail-list">{history.map((item) => <li key={item.redemptionId}><div><strong>{item.rewardName}</strong><span>{t("reward.coinPaidValue", { cost: item.coinCostPaid })}</span><span>{t("reward.redeemed", { date: formatRewardDate(item.redeemedAtUtc) })}</span><span>{item.refundedAtUtc ? t("reward.refunded", { date: formatRewardDate(item.refundedAtUtc) }) : t("reward.notRefunded")}</span></div>{item.refundId === null ? <button className="secondary-action" onClick={(event) => beginRefund(item, event)} type="button">{t("reward.refund")}</button> : <span aria-label={t("reward.refundedStatus")} className="safe-note">{t("reward.refundedStatus")}</span>}</li>)}</ul> : null}</div></details>
...
<div><h3>相关题目</h3>{detail.relatedProblems.length === 0 ? <p>暂无。</p> : <ul>{detail.relatedProblems.map((problem) => <li key={problem.problemId}><button className="list-link" onClick={() => navigate(`/problems/id/${problem.problemId}`)} type="button"><strong>{displayProblemTitle("", problem.title)}</strong></button></li>)}</ul>}</div>
        </section>
      ) : null}
    </>
  );
    ...
    {snapshot ? <>
      ...
      {snapshot.entries.length === 0 ? <section className="empty-state"><h2>{t("today.noTasks")}</h2><p>{t("today.completeBlocksOnly")}</p></section> :
        <ol className="today-list">{snapshot.entries.map((entry, index) => <li className={`today-entry today-entry--${entry.status}`} data-entry-id={entry.entryId} key={entry.entryId} onKeyDown={(event) => { if (event.altKey && event.key === "ArrowUp") { event.preventDefault(); void move(index, -1); } if (event.altKey && event.key === "ArrowDown") { event.preventDefault(); void move(index, 1); } }} tabIndex={0}>
          ...
          <div className="today-entry__body"><div><span className="today-lane">{todayLaneLabel(entry.lane)}</span><span className={`today-status today-status--${entry.status}`}>{todayStatusLabel(entry.status)}</span>{entry.origin === "manual" ? <span className="today-origin">{t("today.manual")}</span> : null}</div><button className="today-problem-link" onClick={() => navigate(entry.reviewAttemptId && entry.status === "inProgress" ? `/review/${entry.reviewAttemptId}` : `/problems/id/${entry.problemId}`)} type="button"><strong>{displayProblemTitle("", entry.problemTitle)}</strong><span>{entry.problemRating === null ? t("today.unrated") : t("today.rating", { rating: entry.problemRating })} · {todayReasonLabel(entry.reason)} · {entry.planningCostMinutes} 分钟</span></button></div>
      ...
      {suggestions && suggestions.suggestions.length > 0 ? <section className="today-suggestions"><h2>额外建议</h2><p>还剩 {suggestions.remainingBudgetMinutes} 分钟。未经确认不会自动加入。</p><ul>{suggestions.suggestions.map((item) => <li key={item.problemId}><span><strong>{displayProblemTitle("", item.problemTitle)}</strong><small>{item.problemRating === null ? "未评级" : `评分 ${item.problemRating}`} · {todayReasonLabel(item.reason)} · {item.planningCostMinutes} 分钟</small></span><button className="secondary-action" disabled={busyEntry === item.problemId} onClick={() => void acceptSuggestion(item.problemId)} type="button">加入今日计划</button></li>)}</ul></section> : null}
    </> : null}
...
function ContestLibraryPage({ navigate }: { navigate: Navigate }) {
  ...
  const [createFamilyDraft, setCreateFamilyDraft] = useState("");
  const [createSeriesDraft, setCreateSeriesDraft] = useState("");
  const [createSeriesFamilyId, setCreateSeriesFamilyId] = useState<number | null>(null);
  const [createYearDraft, setCreateYearDraft] = useState("");
  ...
  const restoreCreationFocus = useCallback((mode: "category" | "series" | "year") => {
    const trigger = mode === "category" ? categoryCreateTriggerRef : mode === "series" ? seriesCreateTriggerRef : yearCreateTriggerRef;
    queueMicrotask(() => trigger.current?.focus());
  }, []);
  const closeCreationDialog = useCallback(() => {
    if (creationPendingRef.current || creationMode === null) return;
    const mode = creationMode;
    setCreationMode(null);
    restoreCreationFocus(mode);
  }, [creationMode, restoreCreationFocus]);
  const openCreationDialog = (mode: "category" | "series" | "year") => {
    setManagementMessage(null);
    if (mode === "category") setCreateFamilyDraft("");
    if (mode === "series") { setCreateSeriesDraft(""); setCreateSeriesFamilyId(familyId); }
    if (mode === "year") setCreateYearDraft("");
    setCreationMode(mode);
  };
  useEffect(() => {
    if (creationMode === null) return;
    const initialFocus = creationMode === "category" ? categoryCreateInputRef.current : creationMode === "series" ? seriesCreateFamilyRef.current : yearCreateInputRef.current;
    initialFocus?.focus();
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") { if (!creationPendingRef.current) { event.preventDefault(); closeCreationDialog(); } return; }
      if (event.key !== "Tab") return;
  ...
  const createFamily = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (creationPendingRef.current) return;
    creationPendingRef.current = true; setManagementBusy(true); setManagementMessage(null);
    try { const created = await createContestLibraryFamily(createFamilyDraft); await refreshFamilies(created.familyId); setCreateFamilyDraft(""); setCreationMode(null); restoreCreationFocus("category"); }
  ...
  const createSeries = async (event: FormEvent<HTMLFormElement>) => {
    ...
    creationPendingRef.current = true; setManagementBusy(true); setManagementMessage(null);
    try {
      await createContestLibrarySeries(createSeriesFamilyId, createSeriesDraft);
      setCreateSeriesDraft("");
      if (familyId === createSeriesFamilyId) setSeries(await listContestLibrarySeries(createSeriesFamilyId));
      setCreationMode(null); restoreCreationFocus("series");
    }
    catch (error) { setManagementMessage(contestLibraryErrorMessage(error)); }
    finally { creationPendingRef.current = false; setManagementBusy(false); }
  };
  const createYear = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (creationPendingRef.current) return;
    creationPendingRef.current = true; setManagementBusy(true); setManagementMessage(null);
    try { const created = await createContestLibraryYear(Number(createYearDraft)); setYearEntities(await listContestLibraryYearEntities()); setCreateYearDraft(""); setCreationMode(null); setYearFilter({ kind: "exact", yearId: created.yearId }); restoreCreationFocus("year"); }
    catch (error) { setManagementMessage(contestLibraryErrorMessage(error)); }
    finally { creationPendingRef.current = false; setManagementBusy(false); }
  };
            ...
            <div aria-label={t("contest.retrySnapshots")} className="contest-shelf-maintenance">
              {items.slice(0, CONTEST_CABINET_CAPACITY).filter((item) => !item.archived && item.importStatus === "incomplete").map((item) => (
                <button className="secondary-action" disabled={importing} key={item.contestId} onClick={() => void retryMissing(item.contestId)} type="button">
                  {t("contest.retrySnapshotsFor", { title: item.title })}
                </button>
              ))}
            </div>
          ) : null}
...
function ContestDetail({ contestId, navigate }: { contestId: number; navigate: Navigate }) {
  ...
  return <>
    <PageHeader eyebrow="M7 · 比赛事实" headingRef={headingRef} title={detail.title} />
