...
mod tests {
    ...
    use super::{
        app_shell_status_dto, custom_reward_status, custom_reward_summary_dto,
        ...
        workspace_error_dto, workspace_status_dto, CompleteReviewInput,
    ...
    #[test]
    fn reward_account_serializes_frontend_fields() {
        let dto = reward_account_summary_dto(acm_os_application::RewardAccountSummary {
            xp_balance: 42,
            coin_balance: 17,
            level: 3,
        });
    ...
    #[test]
    fn custom_reward_and_history_serialization_preserve_projection() {
        let reward = custom_reward_summary_dto(acm_os_application::CustomRewardSummary {
        ...
        assert_eq!(
            ...
            json!({"customRewardId": "reward", "name": "Coffee", "coinCost": 25, "status": "active"})
        );
        let unrefunded = redemption_history_item_dto(acm_os_application::RedemptionHistoryItem {
            redemption_id: "redemption".into(),
            custom_reward_id: "reward".into(),
            reward_name: "Coffee".into(),
            coin_cost_paid: 25,
            redeemed_at_utc: "2026-08-25T00:00:00Z".into(),
            refund_id: None,
            refunded_at_utc: None,
        });
        assert_eq!(
            serde_json::to_value(unrefunded).unwrap()["refundId"],
            json!(null)
        );
        let refunded = redemption_history_item_dto(acm_os_application::RedemptionHistoryItem {
            redemption_id: "redemption".into(),
            custom_reward_id: "reward".into(),
            reward_name: "Renamed Coffee".into(),
            coin_cost_paid: 25,
            redeemed_at_utc: "2026-08-25T00:00:00Z".into(),
            refund_id: Some("refund".into()),
            refunded_at_utc: Some("2026-08-25T01:00:00Z".into()),
        });
        assert_eq!(
            serde_json::to_value(&refunded).unwrap()["coinCostPaid"],
            json!(25)
        );
        assert_eq!(
            serde_json::to_value(&refunded).unwrap()["refundId"],
            json!("refund")
        );
    }

    #[test]
    fn reward_dispositions_and_errors_have_stable_codes() {
        assert_eq!(
            redemption_disposition(acm_os_application::RedemptionDisposition::Processed),
            "processed"
        );
        assert_eq!(
            redemption_disposition(acm_os_application::RedemptionDisposition::AlreadyProcessed),
            "alreadyProcessed"
        );
        assert_eq!(
        ...
        assert_eq!(
            custom_reward_status(acm_os_application::CustomRewardStatus::Archived),
            "archived"
        );
        assert_eq!(
            acm_os_application::RewardError::InsufficientCoin.code(),
            "insufficient_coin"
        );
        assert_eq!(
            acm_os_application::RewardError::RedemptionIntentConflict.code(),
            "redemption_intent_conflict"
        );
    }

    #[test]
    fn reward_mutation_dtos_preserve_intent_identity_without_generation() {
        let redemption_id = "018f0d8e-4a5b-7c6d-8e9f-0123456789ab";
        let refund_id = "018f0d8e-4a5b-7c6d-8e9f-0123456789ac";
        assert_eq!(validate_reward_intent_id(redemption_id), Ok(()));
        assert_eq!(validate_reward_intent_id(refund_id), Ok(()));
        assert_eq!(validate_reward_intent_id("short"), Err("invalid_identity"));
        let redeem = redemption_result_dto(acm_os_application::RedemptionResult {
            disposition: acm_os_application::RedemptionDisposition::AlreadyProcessed,
        ...
        assert_eq!(
            serde_json::to_value(redeem).unwrap()["redemptionId"],
            json!(redemption_id)
        );
        let refund = refund_result_dto(acm_os_application::RefundResult {
            disposition: acm_os_application::RefundDisposition::Processed,
            refund_id: refund_id.into(),
            redemption_id: redemption_id.into(),
            refunded_at_utc: "2026-08-25T01:00:00Z".into(),
        });
        assert_eq!(
            serde_json::to_value(refund).unwrap()["refundId"],
            json!(refund_id)
        );
    }
